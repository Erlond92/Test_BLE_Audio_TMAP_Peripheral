...
typedef uint8_t APP_PASyncState_t;
...
#define APP_BIG_SYNC_STATE_SYNCHRONIZING (0x01)
#define APP_BIG_SYNC_STATE_SYNCHRONIZED  (0x02)

typedef uint8_t APP_ScanState_t;
#define APP_SCAN_STATE_IDLE              (0x00)
#define APP_SCAN_STATE_SCANNING          (0x01)
...
#define APP_BROADCAST_MODE_SINK_ONLY                            (0x01)
#define APP_BROADCAST_MODE_SCAN_DELEGATOR                       (0x02)
...
typedef struct
{
  ...
  uint8_t                       codec_specific_config_bis[2][MAX_BSNK_BIS_CODEC_CONFIG_SIZE];
  uint8_t                       codec_specific_config_subgroup[2][MAX_BSNK_SUBGROUP_CODEC_CONFIG_SIZE];
  uint8_t                       RTN;
  uint16_t                      max_transport_latency;
  uint8_t                       subgroup_metadata[2][MAX_BSNK_METADATA_LEN];
  Target_Phy_t                  phy;
  uint8_t                       encryption;
  uint32_t                      broadcast_code[4u];
  uint8_t                       audio_driver_config;
  uint16_t                      current_BIS_conn_handles[APP_MAX_NUM_BIS];
  uint8_t                       current_num_bis;
  BAP_AudioCodecController_t    AudioCodecInController;
  BAP_SupportedStandardCodec_t  aStandardCodec[1];
  APP_PASyncState_t             PASyncState;
  APP_BIGSyncState_t            BIGSyncState;
  APP_ScanState_t               ScanState;
  uint8_t                       PASyncHandle;
  Audio_Location_t              Audio_Location;
  uint8_t                       sync_bis_index[APP_MAX_NUM_BIS];
  uint8_t                       num_sync_bis;
} APP_BNSK_Context_t;
