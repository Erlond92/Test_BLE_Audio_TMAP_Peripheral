...
/*Delay of Timer used to relaunch a CAP Linkup procedure*/
#define LINKUP_RETRY_TIMEOUT                    (2000u)
...
/*Timer Object used to relaunch a CAP Linkup procedure*/
static UTIL_TIMER_Object_t LinkupRetry_Timer;
...
static void LinkupRetry_TimerCallback(void *arg);
...
tBleStatus TMAPAPP_Linkup(uint16_t ConnHandle)
{
  ...
  if (p_conn != 0)
  {
    ...
    if (NVMLink != 0)
    {
      ...
      status = CAP_Linkup(ConnHandle,NVMLink,0x00);
...
void TMAPAPP_AclConnected(uint16_t ConnHandle, uint8_t Peer_Address_Type, uint8_t Peer_Address[6], uint8_t role)
{
  ...
  if (p_conn != 0)
  {
    ...
    {
      tBleStatus status;
      uint8_t enable_solicitation_request = 0;
      TMAPAPP_StopAdvertising();

      if (TMAPAPP_Context.BroadcastMode == APP_BROADCAST_MODE_SCAN_DELEGATOR)
      {
        enable_solicitation_request = 1;
      }

      /* Start Advertising */
      if (TMAPAPP_Context.CSIPRank > 0)
      {
        status = TMAPAPP_StartAdvertising(CAP_GENERAL_ANNOUNCEMENT, enable_solicitation_request, GAP_APPEARANCE_EARBUD);
      }
      else
      {
      ...
      */
      if (enable_ongoing == 0u)
      {
        for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
        {
          ...
          {
            p_other_conn = &TMAPAPP_Context.ACL_Conn[conn];
            if ((p_other_conn->AvailableSnkAudioContext != TMAPAPP_Context.AvailableSnkAudioContext) \
                || (p_other_conn->AvailableSrcAudioContext != TMAPAPP_Context.AvailableSrcAudioContext))
            {
              ...
              LOG_INFO_APP("Send Update Available Snk Audio Contexts 0x%04X and Available Src Audio Contexts 0x%04X with CAP Initiator on ConnHandle 0x%04X returns status 0x%02X\n",
                           ...
                           p_other_conn->Acl_Conn_Handle,
                           status);
              p_other_conn->AvailableSnkAudioContext = TMAPAPP_Context.AvailableSnkAudioContext;
              p_other_conn->AvailableSrcAudioContext = TMAPAPP_Context.AvailableSrcAudioContext;
              UNUSED(status);
            }
            else
...
static tBleStatus CAPAPP_Init(Audio_Role_t AudioRole, uint8_t csip_config_id)
{
  ...
  APP_MCP_Config.Clt.OptionFeatures = APP_MCP_CLT_FEATURE_OPTIONS;
  ...
  for (uint8_t conn = 0; conn< CFG_BLE_NUM_LINK; conn++)
  {
    ...
    TMAPAPP_Context.ACL_Conn[conn].AvailableSnkAudioContext = 0x0000u;
    TMAPAPP_Context.ACL_Conn[conn].AvailableSrcAudioContext = 0x0000u;
  }
  ...
  TMAPAPP_Context.BSNK.base_subgroups[0].pBIS = &(TMAPAPP_Context.BSNK.base_bis[0]);
  ...
  TMAPAPP_Context.BSNK.base_subgroups[1].pBIS = &(TMAPAPP_Context.BSNK.base_bis[1]);
...
#if (APP_NUM_SRC_PAC_RECORDS > 0u)
  /* 1 Audio Source Mandatory codec specific capabilities */
  num_src_capabilities = MIN_NUM_BAP_SRC_CODEC_CAP;
#endif /* (APP_NUM_SRC_PAC_RECORDS > 0u) */
  ...
  if (audio_role & AUDIO_ROLE_SINK)
  {
    ...
    /*Set Codec ID configuration*/
    TMAPAPP_Context.aSnkPACRecord[0].Cap.CodecID.CodingFormat = AUDIO_CODING_FORMAT_LC3;
    TMAPAPP_Context.aSnkPACRecord[0].Cap.CodecID.CompanyID = COMPANY_ID;
    TMAPAPP_Context.aSnkPACRecord[0].Cap.CodecID.VsCodecID = 0x0000;

    /* Set supported codec capabilities configuration */
    TMAPAPP_Context.aSnkCap[0][0] = 0x03;
    TMAPAPP_Context.aSnkCap[0][1] = CODEC_CAP_SAMPLING_FREQ;
    for (uint8_t i = 0; i < num_snk_capabilities ;i++)
    {
      TMAPAPP_Context.aSnkCap[0][2] |= (uint8_t) (APP_CodecSpecificCap[a_snk_cap_id[i]].Freq);
      TMAPAPP_Context.aSnkCap[0][3] |= (uint8_t) ((APP_CodecSpecificCap[a_snk_cap_id[i]].Freq >> 8 ));
    }

    TMAPAPP_Context.aSnkCap[0][4] = 0x02;
    TMAPAPP_Context.aSnkCap[0][5] = CODEC_CAP_FRAME_DURATION;
    for (uint8_t i = 0; i < num_snk_capabilities ;i++)
    {
      TMAPAPP_Context.aSnkCap[0][6] |= APP_CodecSpecificCap[a_snk_cap_id[i]].FrameDuration;
    }

    TMAPAPP_Context.aSnkCap[0][7] = 0x05;
    TMAPAPP_Context.aSnkCap[0][8] = CODEC_CAP_OCTETS_PER_CODEC_FRAME;
    for (uint8_t i = 0; i < num_snk_capabilities ;i++)
    {
      ...
      if (i > 0)
      {
        min_octet_per_codec_frame = MIN(min_octet_per_codec_frame,
                                        TMAPAPP_Context.aSnkCap[0][9] + (TMAPAPP_Context.aSnkCap[0][10] << 8));
        max_octet_per_codec_frame = MAX(max_octet_per_codec_frame,
                                        TMAPAPP_Context.aSnkCap[0][11] + (TMAPAPP_Context.aSnkCap[0][12] << 8));
      }

      TMAPAPP_Context.aSnkCap[0][9] = (uint8_t) (min_octet_per_codec_frame);
      TMAPAPP_Context.aSnkCap[0][10] = (uint8_t) (min_octet_per_codec_frame >> 8 );
      TMAPAPP_Context.aSnkCap[0][11] = (uint8_t) (max_octet_per_codec_frame);
      TMAPAPP_Context.aSnkCap[0][12] = (uint8_t) (max_octet_per_codec_frame >> 8);
    }

    TMAPAPP_Context.aSnkCap[0][13] = 0x02;
    TMAPAPP_Context.aSnkCap[0][14] = CODEC_CAP_AUDIO_CHNL_COUNTS;
    TMAPAPP_Context.aSnkCap[0][15] = (APP_MAX_CHANNEL_PER_SNK_ASE | SUPPORTED_AUDIO_CHNL_COUNT_1);

    TMAPAPP_Context.aSnkPACRecord[0].Cap.pSpecificCap = &TMAPAPP_Context.aSnkCap[0][0];
    TMAPAPP_Context.aSnkPACRecord[0].Cap.SpecificCapLength = 16u;

    /* metadata in Audio Codec Configuration*/
    TMAPAPP_Context.aSnkPACRecord[0].Cap.MetadataLength = 0x04;
    TMAPAPP_Context.aSnkMetadata[0][0] = METADATA_PREFERRED_AUDIO_CONTEXTS_LENGTH;
    TMAPAPP_Context.aSnkMetadata[0][1] = METADATA_PREFERRED_AUDIO_CONTEXTS;
    TMAPAPP_Context.aSnkMetadata[0][2] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL);
    TMAPAPP_Context.aSnkMetadata[0][3] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL >> 8 );
    TMAPAPP_Context.aSnkPACRecord[0].Cap.pMetadata = &TMAPAPP_Context.aSnkMetadata[0][0];

    status = CAP_RegisterSnkPACRecord(&TMAPAPP_Context.aSnkPACRecord[0],&TMAPAPP_Context.aSnkPACRecordHandle[0]);
    if (status == BLE_STATUS_SUCCESS)
    {
  ...
  if (audio_role & AUDIO_ROLE_SOURCE)
  {
    ...
    /*Set Codec ID configuration*/
    TMAPAPP_Context.aSrcPACRecord[0].Cap.CodecID.CodingFormat = AUDIO_CODING_FORMAT_LC3;
    TMAPAPP_Context.aSrcPACRecord[0].Cap.CodecID.CompanyID = COMPANY_ID;
    TMAPAPP_Context.aSrcPACRecord[0].Cap.CodecID.VsCodecID = 0x0000;

    /* Set supported codec capabilities configuration */
    TMAPAPP_Context.aSrcCap[0][0] = 0x03;
    TMAPAPP_Context.aSrcCap[0][1] = CODEC_CAP_SAMPLING_FREQ;
    for (uint8_t i = 0; i < num_src_capabilities ;i++)
    {
      TMAPAPP_Context.aSrcCap[0][2] |= (uint8_t) (APP_CodecSpecificCap[a_src_cap_id[i]].Freq);
      TMAPAPP_Context.aSrcCap[0][3] |= (uint8_t) ((APP_CodecSpecificCap[a_src_cap_id[i]].Freq >> 8 ));
    }

    TMAPAPP_Context.aSrcCap[0][4] = 0x02;
    TMAPAPP_Context.aSrcCap[0][5] = CODEC_CAP_FRAME_DURATION;
    for (uint8_t i = 0; i < num_src_capabilities ;i++)
    {
      TMAPAPP_Context.aSrcCap[0][6] |= APP_CodecSpecificCap[a_src_cap_id[i]].FrameDuration;
    }

    TMAPAPP_Context.aSrcCap[0][7] = 0x05;
    TMAPAPP_Context.aSrcCap[0][8] = CODEC_CAP_OCTETS_PER_CODEC_FRAME;
    for (uint8_t i = 0; i < num_src_capabilities ;i++)
    {
      ...
      {
        min_octet_per_codec_frame = MIN(min_octet_per_codec_frame,
                                        TMAPAPP_Context.aSrcCap[0][9] + (TMAPAPP_Context.aSrcCap[0][10] << 8));
        max_octet_per_codec_frame = MAX(max_octet_per_codec_frame,
                                        TMAPAPP_Context.aSrcCap[0][11] + (TMAPAPP_Context.aSrcCap[0][12] << 8));
      }

      TMAPAPP_Context.aSrcCap[0][9] = (uint8_t) (min_octet_per_codec_frame);
      TMAPAPP_Context.aSrcCap[0][10] = (uint8_t) (min_octet_per_codec_frame >> 8 );
      TMAPAPP_Context.aSrcCap[0][11] = (uint8_t) (max_octet_per_codec_frame);
      TMAPAPP_Context.aSrcCap[0][12] = (uint8_t) (max_octet_per_codec_frame >> 8);
    }

    TMAPAPP_Context.aSrcCap[0][13] = 0x02;
    TMAPAPP_Context.aSrcCap[0][14] = CODEC_CAP_AUDIO_CHNL_COUNTS;
    TMAPAPP_Context.aSrcCap[0][15] = (APP_MAX_CHANNEL_PER_SRC_ASE | SUPPORTED_AUDIO_CHNL_COUNT_1);

    TMAPAPP_Context.aSrcPACRecord[0].Cap.pSpecificCap = &TMAPAPP_Context.aSrcCap[0][0];
    TMAPAPP_Context.aSrcPACRecord[0].Cap.SpecificCapLength = 16u;

    /* metadata in Audio Codec Configuration*/
    TMAPAPP_Context.aSrcPACRecord[0].Cap.MetadataLength = 0x04;
    TMAPAPP_Context.aSrcMetadata[0][0] = METADATA_PREFERRED_AUDIO_CONTEXTS_LENGTH;
    TMAPAPP_Context.aSrcMetadata[0][1] = METADATA_PREFERRED_AUDIO_CONTEXTS;
    TMAPAPP_Context.aSrcMetadata[0][2] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL);
    TMAPAPP_Context.aSrcMetadata[0][3] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL >> 8 );
    TMAPAPP_Context.aSrcPACRecord[0].Cap.pMetadata = &TMAPAPP_Context.aSrcMetadata[0][0];

    status = CAP_RegisterSrcPACRecord(&TMAPAPP_Context.aSrcPACRecord[0],&TMAPAPP_Context.aSrcPACRecordHandle[0]);
    if (status == BLE_STATUS_SUCCESS)
    {
  ...
  /* Set Available Audio Contexts for reception and transmission */
  ...
  status = CAP_RegisterAudioChannels(APP_NUM_SNK_ASE,APP_NUM_SRC_ASE);
  if (status == BLE_STATUS_SUCCESS)
  {
    ...
    TMAPAPP_Context.NumSnkASEs = APP_NUM_SNK_ASE;
    TMAPAPP_Context.NumSrcASEs = APP_NUM_SRC_ASE;
  }
  else
  {
...
#endif /*((APP_TMAP_ROLE & TMAP_ROLE_BROADCAST_MEDIA_RECEIVER) == TMAP_ROLE_BROADCAST_MEDIA_RECEIVER) */
  /*Register the Task dedicated to retry a Linkup procedure*/
  UTIL_SEQ_RegTask( 1u <<CFG_TASK_APP_LINKUP_RETRY_TIMER_ID, UTIL_SEQ_RFU,APP_LinkupTaskHandler);
...
static tBleStatus TMAPAPP_TMAPInit(uint16_t Role)
{
  ...
  uint8_t a_ct_snk_pac_id[MIN_NUM_TMAP_CT_SRC_CODEC_CAP] = TMAP_MANDATORY_CT_SRC_CODEC_CAP_LIST;
  uint8_t a_mr_snk_pac_id[MIN_NUM_TMAP_MR_SNK_CODEC_CAP] = TMAP_MANDATORY_MR_SNK_CODEC_CAP_LIST;
#endif /*(APP_NUM_SNK_PAC_RECORDS > 0)*/
#if (APP_NUM_SRC_PAC_RECORDS > 0)
  uint8_t src_record_i = MIN_NUM_BAP_SRC_PAC_RECORDS;
  uint8_t a_ct_src_pac_id[MIN_NUM_TMAP_CT_SNK_CODEC_CAP] = TMAP_MANDATORY_CT_SNK_CODEC_CAP_LIST;
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    if (tmap_config.Role & TMAP_ROLE_CALL_TERMINAL)
    {
      ...
      for (i = 0; i < MIN_NUM_TMAP_CT_SNK_CODEC_CAP ;i++)
      {
      ...
      for (i = 0; i < MIN_NUM_TMAP_CT_SNK_CODEC_CAP ;i++)
      {
        uint32_t min_octet_per_codec_frame = APP_CodecSpecificCap[a_ct_snk_pac_id[i]].MinOctetsPerCodecFrame;
        uint32_t max_octet_per_codec_frame = APP_CodecSpecificCap[a_ct_snk_pac_id[i]].MaxOctetsPerCodecFrame;

        if (i > 0)
        {
          min_octet_per_codec_frame = MIN(min_octet_per_codec_frame,
                                          TMAPAPP_Context.aSnkCap[snk_record_i][9] + (TMAPAPP_Context.aSnkCap[snk_record_i][10] << 8));
          max_octet_per_codec_frame = MAX(max_octet_per_codec_frame,
                                          TMAPAPP_Context.aSnkCap[snk_record_i][11] + (TMAPAPP_Context.aSnkCap[snk_record_i][12] << 8));
        }

        TMAPAPP_Context.aSnkCap[snk_record_i][9] = (uint8_t) (min_octet_per_codec_frame);
        TMAPAPP_Context.aSnkCap[snk_record_i][10] = (uint8_t) (min_octet_per_codec_frame >> 8 );
        TMAPAPP_Context.aSnkCap[snk_record_i][11] = (uint8_t) (max_octet_per_codec_frame);
        TMAPAPP_Context.aSnkCap[snk_record_i][12] = (uint8_t) (max_octet_per_codec_frame >> 8);
      }

      TMAPAPP_Context.aSnkCap[snk_record_i][13] = 0x02;
      TMAPAPP_Context.aSnkCap[snk_record_i][14] = CODEC_CAP_AUDIO_CHNL_COUNTS;
      TMAPAPP_Context.aSnkCap[snk_record_i][15] = (APP_MAX_CHANNEL_PER_SNK_ASE | SUPPORTED_AUDIO_CHNL_COUNT_1);
      ...
      /* metadata in Audio Codec Configuration*/
      TMAPAPP_Context.aSnkPACRecord[snk_record_i].Cap.MetadataLength = 0x04;
      TMAPAPP_Context.aSnkMetadata[snk_record_i][0] = METADATA_PREFERRED_AUDIO_CONTEXTS_LENGTH;
      TMAPAPP_Context.aSnkMetadata[snk_record_i][1] = METADATA_PREFERRED_AUDIO_CONTEXTS;
      TMAPAPP_Context.aSnkMetadata[snk_record_i][2] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL);
      TMAPAPP_Context.aSnkMetadata[snk_record_i][3] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL >> 8 );
      TMAPAPP_Context.aSnkPACRecord[snk_record_i].Cap.pMetadata = &TMAPAPP_Context.aSnkMetadata[snk_record_i][0];

      ret = CAP_RegisterSnkPACRecord(&TMAPAPP_Context.aSnkPACRecord[snk_record_i],
                                     &TMAPAPP_Context.aSnkPACRecordHandle[snk_record_i]);
      if (ret == BLE_STATUS_SUCCESS)
      {
        LOG_INFO_APP("Successfully registered Sink PAC Record id %d\n",snk_record_i);
        snk_record_i++;
      ...
      for (i = 0; i < MIN_NUM_TMAP_CT_SRC_CODEC_CAP ;i++)
      {
      ...
      for (i = 0; i < MIN_NUM_TMAP_CT_SRC_CODEC_CAP ;i++)
      {
        uint32_t min_octet_per_codec_frame = APP_CodecSpecificCap[a_ct_src_pac_id[i]].MinOctetsPerCodecFrame;
        uint32_t max_octet_per_codec_frame = APP_CodecSpecificCap[a_ct_src_pac_id[i]].MaxOctetsPerCodecFrame;

        if (i > 0)
        {
          min_octet_per_codec_frame = MIN(min_octet_per_codec_frame,
                                          TMAPAPP_Context.aSrcCap[src_record_i][9] + (TMAPAPP_Context.aSrcCap[src_record_i][10] << 8));
          max_octet_per_codec_frame = MAX(max_octet_per_codec_frame,
                                          TMAPAPP_Context.aSrcCap[src_record_i][11] + (TMAPAPP_Context.aSrcCap[src_record_i][12] << 8));
        }

        TMAPAPP_Context.aSrcCap[src_record_i][9] = (uint8_t) (min_octet_per_codec_frame);
        TMAPAPP_Context.aSrcCap[src_record_i][10] = (uint8_t) (min_octet_per_codec_frame >> 8 );
        TMAPAPP_Context.aSrcCap[src_record_i][11] = (uint8_t) (max_octet_per_codec_frame);
        TMAPAPP_Context.aSrcCap[src_record_i][12] = (uint8_t) (max_octet_per_codec_frame >> 8);
      }

      TMAPAPP_Context.aSrcCap[src_record_i][13] = 0x02;
      TMAPAPP_Context.aSrcCap[src_record_i][14] = CODEC_CAP_AUDIO_CHNL_COUNTS;
      TMAPAPP_Context.aSrcCap[src_record_i][15] = (APP_MAX_CHANNEL_PER_SRC_ASE | SUPPORTED_AUDIO_CHNL_COUNT_1);
      ...
      /* metadata in Audio Codec Configuration*/
      TMAPAPP_Context.aSrcPACRecord[src_record_i].Cap.MetadataLength = 0x04;
      TMAPAPP_Context.aSrcMetadata[src_record_i][0] = METADATA_PREFERRED_AUDIO_CONTEXTS_LENGTH;
      TMAPAPP_Context.aSrcMetadata[src_record_i][1] = METADATA_PREFERRED_AUDIO_CONTEXTS;
      TMAPAPP_Context.aSrcMetadata[src_record_i][2] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL);
      TMAPAPP_Context.aSrcMetadata[src_record_i][3] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL >> 8 );
      TMAPAPP_Context.aSrcPACRecord[src_record_i].Cap.pMetadata = &TMAPAPP_Context.aSrcMetadata[src_record_i][0];

      ret = CAP_RegisterSrcPACRecord(&TMAPAPP_Context.aSrcPACRecord[src_record_i],
                                     &TMAPAPP_Context.aSrcPACRecordHandle[src_record_i]);
      if (ret == BLE_STATUS_SUCCESS)
      {
        LOG_INFO_APP("Successfully registered Source PAC Record id %d\n",src_record_i);
        src_record_i++;
    ...
    if (tmap_config.Role & (TMAP_ROLE_UNICAST_MEDIA_RECEIVER | TMAP_ROLE_BROADCAST_MEDIA_RECEIVER))
    {
      ...
      TMAPAPP_Context.aSnkCap[snk_record_i][0] = 0x03;
      TMAPAPP_Context.aSnkCap[snk_record_i][1] = CODEC_CAP_SAMPLING_FREQ;
      for (i = 0; i < MIN_NUM_TMAP_MR_SNK_CODEC_CAP; i++)
      {
      ...
      for (i = 0; i < MIN_NUM_TMAP_MR_SNK_CODEC_CAP; i++)
      {
        uint32_t min_octet_per_codec_frame = APP_CodecSpecificCap[a_mr_snk_pac_id[i]].MinOctetsPerCodecFrame;
        uint32_t max_octet_per_codec_frame = APP_CodecSpecificCap[a_mr_snk_pac_id[i]].MaxOctetsPerCodecFrame;
      ...
      TMAPAPP_Context.aSnkMetadata[snk_record_i][0] = METADATA_PREFERRED_AUDIO_CONTEXTS_LENGTH;
      TMAPAPP_Context.aSnkMetadata[snk_record_i][1] = METADATA_PREFERRED_AUDIO_CONTEXTS;
      TMAPAPP_Context.aSnkMetadata[snk_record_i][2] = (uint8_t) (AUDIO_CONTEXT_MEDIA);
      TMAPAPP_Context.aSnkMetadata[snk_record_i][3] = (uint8_t) (AUDIO_CONTEXT_MEDIA >> 8 );
      TMAPAPP_Context.aSnkPACRecord[snk_record_i].Cap.pMetadata = &TMAPAPP_Context.aSnkMetadata[snk_record_i][0];
  ...
  switch(pNotification->EvtOpcode)
  {
    case CAP_LINKUP_COMPLETE_EVT:
    {
      ...
      if (p_conn != 0)
      {
        if ( current_link == 0)
        {
          if (p_conn->LinkupRetry == 0u)
          {
            /*No service has been discovery, we will retry a CAP Linkup in a moment*/
            p_conn->LinkupRetry = 1u;
            UTIL_TIMER_Create(&LinkupRetry_Timer,
                              LINKUP_RETRY_TIMEOUT,
                              UTIL_TIMER_ONESHOT,
                              &LinkupRetry_TimerCallback,
                              0);
            UTIL_TIMER_Start(&LinkupRetry_Timer);
          }
        }
        if (p_conn->LinkupRetry != 1u)
        {
          /*Check if timer should be started to perform a linkup retry with another remote device*/
          for (uint8_t conn = 0; conn < CFG_BLE_NUM_LINK ; conn++)
          {
            if ( (TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu) \
                  && (TMAPAPP_Context.ACL_Conn[conn].LinkupRetry == 1u))
            {
              UTIL_TIMER_Create(&LinkupRetry_Timer,
                                LINKUP_RETRY_TIMEOUT,
                                UTIL_TIMER_ONESHOT,
    ...
    case CAP_UNICAST_SERVER_ENABLE_REQ_EVT:
    {
      ...
      Audio_Context_t snk_context = TMAPAPP_Context.AvailableSnkAudioContext;
      Audio_Context_t src_context = TMAPAPP_Context.AvailableSrcAudioContext;
      ...
      if (p_conn != 0)
      {
        ...
        else
        {
          ...
          for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
          {
            uint8_t streaming_ongoing = 0u;
            if ((TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu) \
                && (&TMAPAPP_Context.ACL_Conn[conn] != p_conn))
            {
              p_other_conn = &TMAPAPP_Context.ACL_Conn[conn];
#if (APP_NUM_SNK_ASE > 0u)
              for ( uint8_t i = 0; i < TMAPAPP_Context.NumSnkASEs;i++)
    ...
    case CAP_BROADCAST_BASE_REPORT_EVT:
    {
      ...
      if (TMAPAPP_Context.BSNK.PASyncState == APP_PA_SYNC_STATE_SYNCHRONIZED)
      {
        TMAPAPP_Context.BSNK.PASyncState = APP_PA_SYNC_STATE_BASE_RECEIVED;
      ...
      {
        payload_len += index;
        for (i = 0; i < TMAPAPP_Context.BSNK.base_group.NumSubgroups && is_different == 0x00u; i++)
        {
          is_different = CAP_Broadcast_IsBASESubgroupDifferent(base_data->pBasePayload + payload_len,
                                                    ...
                                                    &(index));
          payload_len += index;
          for (j = 0; (j < TMAPAPP_Context.BSNK.base_subgroups[i].NumBISes && is_different == 0x00u); j++)
          {
      ...
      if (is_different == 0x00u)
      {
        /* BASE report is not different from the one in TMAPAPP_Context.BSNK */
...
static void LinkupRetry_TimerCallback(void *arg)
{
  UTIL_SEQ_SetTask(1U << CFG_TASK_APP_LINKUP_RETRY_TIMER_ID, CFG_SEQ_PRIO_0);
}
static void APP_LinkupTaskHandler(void)
{
  tBleStatus status;
  ...
  for (uint8_t conn = 0; conn < CFG_BLE_NUM_LINK ; conn++)
  {
    if ( TMAPAPP_Context.ACL_Conn[conn].LinkupRetry == 1u)
    {
      /*Update LinkupRetry flag to indicate that Linkup retry has been handled*/
      TMAPAPP_Context.ACL_Conn[conn].LinkupRetry = 0xFFu;
      status = TMAPAPP_Linkup(TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle);
      if (status == BLE_STATUS_SUCCESS)
      {
        return;
      }
                                         ...
                                         uint16_t Appearance)
{
  *AdvPacketLen = TMAP_BuildAdvPacket(AnnouncementType,
  ...
  if (EnableSolicitationRequest == 1)
  {
    *AdvPacketLen += CAP_Broadcast_BuildSolicitationRequestAdvPacket(&pAdvData[*AdvPacketLen],
                                                                     CAP_ADV_DATA_LEN+TMAP_USR_ADV_DATA_LEN-*AdvPacketLen);
  }
  ...
  if (p_conn != 0)
  {
    ...
    if(status == 1u)
    {
      ...
      if (pASE->streaming_audio_context != audio_context)
      {
        ...
        /* if context are now available, update them */
        available_snk_context = TMAPAPP_Context.AvailableSnkAudioContext | snk_context;
        available_src_context = TMAPAPP_Context.AvailableSrcAudioContext | src_context;
        ...
        if (TMAPAPP_Context.NumConn > 1u )
        {
          ...
          for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
          {
            if ((TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu) && (&TMAPAPP_Context.ACL_Conn[conn] != p_conn))
            {
              if ((TMAPAPP_Context.ACL_Conn[conn].AvailableSnkAudioContext != TMAPAPP_Context.AvailableSnkAudioContext) \
                  || (TMAPAPP_Context.ACL_Conn[conn].AvailableSrcAudioContext != TMAPAPP_Context.AvailableSrcAudioContext))
              {
                /* we update the Audio Contexts availability to other connected CAP Initiators */
                status = CAP_UpdateAvailableAudioContexts(TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle,
                                                          TMAPAPP_Context.AvailableSnkAudioContext,
                                                          TMAPAPP_Context.AvailableSrcAudioContext);
                LOG_INFO_APP("Send Update Available Snk Audio Contexts 0x%04X and Available Src Audio Contexts 0x%04X with CAP Initiator on ConnHandle 0x%04X returns status 0x%02X\n",
                             TMAPAPP_Context.AvailableSnkAudioContext,
                             ...
                             TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle,
                             status);
                TMAPAPP_Context.ACL_Conn[conn].AvailableSnkAudioContext = TMAPAPP_Context.AvailableSnkAudioContext;
                TMAPAPP_Context.ACL_Conn[conn].AvailableSrcAudioContext = TMAPAPP_Context.AvailableSrcAudioContext;
              }
              else
              {
                                             ...
                                             Audio_Context_t AudioContexts)
{
  ...
  if (Type == ASE_SINK)
  {
    ...
    {
      /*If Audio Contexts includes Media type :  Media, Alerts, Sound Effects and Unspecified Contexts would be no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_MEDIA | AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_ALERTS | AUDIO_CONTEXT_SOUND_EFFECTS));
    }
    if (AudioContexts & AUDIO_CONTEXT_ALERTS)
    {
      /*If Audio Contexts includes Alerts type :  Media, Alerts, Sound Effects and Unspecified Contexts would be no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_ALERTS | AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_MEDIA | AUDIO_CONTEXT_SOUND_EFFECTS));
    }
    ...
    {
      /*If Audio Contexts includes RingTone type :  Ringtone, Media, Alerts, Sound Effects and Unspecified Contexts would be no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_RINGTONE | AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_MEDIA | AUDIO_CONTEXT_ALERTS | AUDIO_CONTEXT_SOUND_EFFECTS));
    }
    if (AudioContexts & AUDIO_CONTEXT_CONVERSATIONAL)
    {
      /*If Audio Contexts includes Conversational type :  Conversational still available but Ringtone, Media, Alerts, Sound Effects and Unspecified Contexts would no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_RINGTONE | AUDIO_CONTEXT_MEDIA | AUDIO_CONTEXT_ALERTS | AUDIO_CONTEXT_SOUND_EFFECTS));
    }
  }
  else
  {
  ...
  if (pASE->type == ASE_SINK)
  {
    ...
    for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
    {
      if (TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu)
      {
        for ( uint8_t i = 0; i < TMAPAPP_Context.NumSnkASEs;i++)
        {
          if ((TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].allocated == 1) \
              && (&TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i] != pASE))
          {
            if ((TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].state == ASE_STATE_ENABLING) \
              || (TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].state == ASE_STATE_STREAMING))
            {
              use_context |= TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].streaming_audio_context;
              available_context &= ~TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].streaming_audio_context;
              LOG_INFO_APP("Streaming Audio Contexts 0x%04X associated to ASE ID %d on ConnHandle 0x%04X still in use\n",
                           TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].streaming_audio_context,
                           TMAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].ID,
                           TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle);
            }
          }
        }
  ...
  if (pASE->type == ASE_SOURCE)
  {
    ...
    for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
    {
      if (TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu)
      {
        for ( uint8_t i = 0; i < TMAPAPP_Context.NumSrcASEs;i++)
        {
          if ((TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].allocated == 1) \
              && (&TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i] != pASE))
          {
            if ((TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].state == ASE_STATE_ENABLING) \
              || (TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].state == ASE_STATE_STREAMING))
            {
              use_context |= TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].streaming_audio_context;
              available_context &= ~TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].streaming_audio_context;
              LOG_INFO_APP("Streaming Audio Contexts 0x%04X associated to ASE ID %d on ConnHandle 0x%04X still in use\n",
                           TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].streaming_audio_context,
                           TMAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].ID,
                           TMAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle);
            }
          }
        }
...
static APP_ACL_Conn_t *APP_AllocateACLConn(uint16_t ConnHandle)
{
  ...
  for (uint8_t i = 0; i < CFG_BLE_NUM_LINK; i++)
  {
    if ( TMAPAPP_Context.ACL_Conn[i].Acl_Conn_Handle == 0xFFFFu)
    {
      ...
      TMAPAPP_Context.ACL_Conn[i].CAPLinkupState = APP_CAP_LINKUP_STATE_NONE;
      TMAPAPP_Context.ACL_Conn[i].LinkupRetry = 0;
      TMAPAPP_Context.ACL_Conn[i].ConfirmIndicationRequired = 0u;
...
static void CCP_MetaEvt_Notification(CCP_Notification_Evt_t *pNotification)
{
  ...
  switch(pNotification->EvtOpcode)
  {
    ...
    case CCP_CLT_OPERATION_COMPLETE_EVT:
    {
      ...
      /* Schedule pending MCP/CCP operation if it exists*/
    ...
    case CCP_CLT_FEATURES_STATUS_EVT:
    {
      ...
      if (info->InbandRingtone == CCP_INBAND_RINGTONE_ENABLED)
      {
         ...
         * If RingTone Audio Contexts is available, the CAP Acceptor doesn't need to generate out-of-band ringtones.
         */
...
static void MCP_MetaEvt_Notification(MCP_Notification_Evt_t *pNotification)
{
  switch(pNotification->EvtOpcode)
  {
    ...
    case MCP_CLT_OPERATION_COMPLETE_EVT:
    {
      ...
      if (p_conn != 0)
      {
        uint16_t CurrentContentCtrlOp = 0;

        LOG_INFO_APP("CurrentContentCtrlOp 0x%08X, PlayPauseOperation : %d, MediaState : %d\n",
                  p_conn->CurrentContentCtrlOp,
                  p_conn->PlayPauseOperation,
                  p_conn->MediaState);
        if (p_conn->CurrentContentCtrlOp & MCP_CLT_OP_CONFIGURE_TRACK_TITLE_NOTIFICATION)
        {
          CurrentContentCtrlOp = MCP_CLT_OP_CONFIGURE_TRACK_TITLE_NOTIFICATION;
        }
        else if (p_conn->CurrentContentCtrlOp & MCP_CLT_OP_READ_MEDIA_STATE)
        {
          CurrentContentCtrlOp = MCP_CLT_OP_READ_MEDIA_STATE;
        }
        else if (p_conn->CurrentContentCtrlOp & MCP_CLT_OP_READ_TRACK_TITLE)
        {
          CurrentContentCtrlOp = MCP_CLT_OP_READ_TRACK_TITLE;
        }
        p_conn->CurrentContentCtrlOp &= ~CurrentContentCtrlOp;
    ...
    case MCP_CLT_OPERATION_TRANSMITTED_EVT:
    {
      ...
      if (p_conn != 0)
      {
        if ((p_event_params->CtrlOp == MCP_MEDIA_CTRL_PLAY) && (p_conn->CurrentContentCtrlOp & MCP_CLT_OP_PLAY_TRACK))
        {
          p_conn->CurrentContentCtrlOp &= ~MCP_CLT_OP_PLAY_TRACK;
        }
        else if ((p_event_params->CtrlOp == MCP_MEDIA_CTRL_PAUSE) && (p_conn->CurrentContentCtrlOp & MCP_CLT_OP_PAUSE_TRACK))
        {
          p_conn->CurrentContentCtrlOp &= ~MCP_CLT_OP_PAUSE_TRACK;
        }
        else if ((p_event_params->CtrlOp == MCP_MEDIA_CTRL_NEXT_TRACK) && (p_conn->CurrentContentCtrlOp & MCP_CLT_OP_MOVE_NEXT_TRACK))
        {
          p_conn->CurrentContentCtrlOp &= ~MCP_CLT_OP_MOVE_NEXT_TRACK;
        }
        else if ((p_event_params->CtrlOp == MCP_MEDIA_CTRL_PREVIOUS_TRACK) && (p_conn->CurrentContentCtrlOp & MCP_CLT_OP_MOVE_PREVIOUS_TRACK))
        {
          p_conn->CurrentContentCtrlOp &= ~MCP_CLT_OP_MOVE_PREVIOUS_TRACK;
        }
        /* schedule pending MCP/CCP operation if it exists*/
...
static void TMAP_SchedulePendingContentControlOp(APP_ACL_Conn_t *pConn)
{
  if (pConn != 0)
  {
    ...
    if (PendingContentCtrlOp != 0x0000)
    {
      LOG_INFO_APP("Content Control Operation 0x%04X on ConnHandle 0x%04X is pending\n",
                   PendingContentCtrlOp,
                   pConn->Acl_Conn_Handle);
    }

#if (APP_MCP_ROLE_CLIENT_SUPPORT == 1u)
    if ((PendingContentCtrlOp & MCP_CLT_OP_CONFIGURE_TRACK_TITLE_NOTIFICATION) \
        && ((pConn->CurrentContentCtrlOp & MCP_CLT_OP_ALL) == 0x0000))
    {
